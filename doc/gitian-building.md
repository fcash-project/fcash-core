...
*Setup instructions for a Gitian build of Fcash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fcash
...
- [Building Fcash Core](#building-fcash-core)
...
Clone the git repositories for fcash and Gitian.
...
in the fcash repository under 'Fetch and create inputs' to install sources which require
...
To build Fcash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fcash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fcash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/fcash-project/fcash-core
...
For example:
```bash
URL=https://github.com/thrasher-/fcash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit fcash=${COMMIT} --url fcash=${URL} ../fcash-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fcash=${COMMIT} --url fcash=${URL} ../fcash-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fcash=${COMMIT} --url fcash=${URL} ../fcash-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fcash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fcash-core/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/fcash-core
SIGPATH=/some/root/path/fcash-detached-sigs

./bin/gbuild --url fcash=${BTCPATH},signature=${SIGPATH} ../fcash-core/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fcash-osx-build.assert
```
