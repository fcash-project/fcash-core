...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/fcash-project/fcash-core/issues>
...
updates will be released anymore. Without security updates, using a fcash
...
ECDSA signatures inside Fcash transactions now use validation using
...
Previous versions of Fcash Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Fcash Core was to raise the default minimum
relay fee.

Fcash Core 0.13.2 will have a strict maximum size on the mempool. The
...
Fcash Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running fcashd without having to do any manual configuration.
...
Fcash Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Fcash Core will relay transactions with
...
entirely in Fcash Core 0.13, and it is at this time undecided whether the
...
Fcash Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Fcash Core automatically creates a hidden service to listen on, without
manual configuration. Fcash Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Fcash Core is listening, and
...
Fcashd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Fcash
Core to use floating fees. By default, Fcash Core will use floating
fees.
...
`fcash.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /fcashrpc>
        ...
        # optional bypass fcashd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< fcashrpc:<password>
    </Location>
...
Restarting fcashd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in fcash.conf
...
For example:
    ...
    $ src/fcash-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Fcash Core code base has
...
- `fcash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `fcash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Fcash scripting system that in combination with
...
Fcash transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Fcash Core.
...
The two phases are distinct in the Fcash-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Fcash wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Fcash block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of fcashs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of fcashs being spent is exactly
...
- **Increased security for multisig:** Fcash addresses (both P2PKH addresses
  ...
  believed to be provided by Fcash's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Fcash's current ledger (called the UTXO set),
  ...
  note that Fcash Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Fcash users to individually opt-in to almost any change in the Fcash
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Fcash Core contributors that may
...
Since Fcash Core 0.10.0, nodes have defaulted to only relaying and
...
- `fcash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `fcash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
